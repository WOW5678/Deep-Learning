...
class StackGCNEncoder(nn.Module):
    ...
    def forward(self, user_supports, item_supports, user_inputs, item_inputs):
        ...
        """
        assert len(user_supports) == len(item_supports) == self.num_support # 都为5
        user_hidden = []
        item_hidden = []
        for i in range(self.num_support):
            tmp_u = torch.matmul(user_inputs, self.weight[i]) #user_inputs：[943,2625], self.weight[i]:[input_dim, output_dim // num_support] 即[2625,500/5]
            tmp_v = torch.matmul(item_inputs, self.weight[i]) #同理可得[1682,100]
            tmp_user_hidden = torch.sparse.mm(user_supports[i], tmp_v)  #user_supports[i]:[943,1682] tmp_user_hidden:[943,100]
            tmp_item_hidden = torch.sparse.mm(item_supports[i], tmp_u)  #item_supports[i]:[1682,943] tmp_item_hidden:[1682,100]
            user_hidden.append(tmp_user_hidden)
            item_hidden.append(tmp_item_hidden)

        user_hidden = torch.cat(user_hidden, dim=1) #列增加 [943,500]
        item_hidden = torch.cat(item_hidden, dim=1) #列增加 [1682,500]
...
class FullyConnected(nn.Module):
    ...
    def forward(self, user_inputs, item_inputs):
        ...
        Args:
            user_inputs (torch.Tensor): 输入的用户特征 [943,41]
        ...
        user_outputs = self.linear_user(user_inputs) #[943,10]
        
        item_inputs = self.dropout(item_inputs)
        item_outputs = self.linear_item(item_inputs) #[1682,10]
...
class Decoder(nn.Module):
    def __init__(self, input_dim, num_weights, num_classes, dropout=0., activation=F.relu):
        ...
        self.weight = nn.Parameter(torch.Tensor(num_weights, input_dim, input_dim)) #[2,75,75]
        self.weight_classifier = nn.Parameter(torch.Tensor(num_weights, num_classes)) #[2,5]
        self.reset_parameters()
    ...
    def forward(self, user_inputs, item_inputs, user_indices, item_indices):
        ...
        user_inputs = self.dropout(user_inputs)
        item_inputs = self.dropout(item_inputs)
        user_inputs = user_inputs[user_indices] #后来[100000,75]
        item_inputs = item_inputs[item_indices] #item_inputs初始[6182,75]，后来[100000,75] ? item_indices 是（100000,）的 没有index out of 的错误么？
        
        basis_outputs = []
        for i in range(self.num_weights):
            tmp = torch.matmul(user_inputs, self.weight[i]) #[100000,75]
            out = torch.sum(tmp * item_inputs, dim=1, keepdim=True) #[100000,1]
            basis_outputs.append(out)

        basis_outputs = torch.cat(basis_outputs, dim=1) #[100000,2]
        
        outputs = torch.matmul(basis_outputs, self.weight_classifier) #[100000,5]
