...
SCORES = torch.tensor([[1, 2, 3, 4, 5]]).float().to(DEVICE)
...
class GraphMatrixCompletion(nn.Module):
    ...
    def forward(self, user_supports, item_supports, 
        ...
        :param user_inputs:
        :param item_inputs:
        :param user_side_inputs:  #[943,41]
        :param item_side_inputs:
        :param user_edge_idx:
        :param item_edge_idx:
        :return:
        '''
        user_gcn, movie_gcn = self.encoder(user_supports, item_supports, user_inputs, item_inputs) #[945,500] [1682,500]
        #即对user节点和movie节点执行一次特征的转换
        user_side_feat, movie_side_feat = self.dense1(user_side_inputs, item_side_inputs) #[943,10],[1682,10]
        
        user_feat = torch.cat((user_gcn, user_side_feat), dim=1)  #列增加 [943,500+10]
        movie_feat = torch.cat((movie_gcn, movie_side_feat), dim=1) #列增加 [1682,500+10]
        
        user_embed, movie_embed = self.dense2(user_feat, movie_feat) # 执行第二次的特征转换 [943,75],[1682,75]
...
def train():
    ...
    for e in range(EPOCHS):
        ...
        # 注意这里的criterion是nn.CrossEntropy()这里的logits是没经过softmax激活的数值
...
def expected_rmse(logits, label):
    ...
    diff = torch.pow(true_y.float() - pred_y, 2)
